...
namespace SymTest
{
    ...
    public sealed class RewriterTests
    {
        ...
        {
            // Pattern: Add(exp, 0)
            Wild wildExp = new Wild("exp");
            IExpression pattern = new Add(ImmutableList.Create<IExpression>(wildExp, new Number(0m)));
            // Replacement: exp
            IExpression replacement = wildExp;
        ...
        private static Rule CreateMulOneRule()
        {
            // Pattern: Mul(exp, 1)
            Wild wildExp = new Wild("exp");
            IExpression pattern = new Multiply(ImmutableList.Create<IExpression>(wildExp, new Number(1m)));
            // Replacement: exp
            IExpression replacement = wildExp;
            return new Rule(pattern, replacement);
        }
        ...
        private static Rule CreateDistributeMultiplyOverAddRule()
        {
            ...
            Wild wildA = new Wild("a");
            Wild wildB = new Wild("b");
            Wild wildC = new Wild("c");
            IExpression pattern = new Multiply(ImmutableList.Create<IExpression>(wildA, new Add(ImmutableList.Create<IExpression>(wildB, wildC))));
            IExpression replacement = new Add(ImmutableList.Create<IExpression>(
                                            new Multiply(ImmutableList.Create<IExpression>(wildA, wildB)),
                                            new Multiply(ImmutableList.Create<IExpression>(wildA, wildC))));
            return new Rule(pattern, replacement);
        }
        ...
        public void Substitute_ReplacesWildsWithBindings()
        {
            ...
            ImmutableDictionary<string, IExpression> bindings = ImmutableDictionary.Create<string, IExpression>()
                .Add("x", new Symbol("A"))
                .Add("y", new Symbol("B"));
        ...
        public void Rewrite_AppliesSingleRuleAtTopLevel()
        {
            ...
            IExpression result = Rewriter.Rewrite(input, ImmutableList.Create<Rule>(rule));
        ...
        public void Rewrite_AppliesRuleRecursivelyIntoArguments()
        {
            ...
            IExpression result = Rewriter.Rewrite(input, ImmutableList.Create<Rule>(addZeroRule, mulOneRule));
            Assert.AreEqual(expected, result);
        }
        ...
        public void Rewrite_AppliesMultipleRulesUntilNoChange()
        {
            ...
            IExpression result = Rewriter.Rewrite(input, ImmutableList.Create<Rule>(addZeroRule, distributeRule));
            Assert.AreEqual(expected, result);
            ...
            IExpression resultLongerAdd = Rewriter.Rewrite(inputLongerAdd, ImmutableList.Create<Rule>(distributeRule));
        ...
        public void Rewrite_TerminatesOnStableExpression()
        {
            ...
            IExpression result = Rewriter.Rewrite(input, ImmutableList<Rule>.Empty);
        ...
        private static bool InternalTryMatchRecursiveAccessor(IExpression expression, IExpression pattern, ImmutableDictionary<string, IExpression>.Builder bindings)
        {
            ...
            Object[] parameters = new Object[] { expression, pattern, bindings };
